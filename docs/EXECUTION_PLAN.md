...
### 4.1 타이머 컴포넌트 구조
- [x] CircularTimer 컴포넌트 생성
- [x] SVG 기반 원형 프로그레스 바 구현
- [x] 중앙 시간 표시 (MM:SS 형식)

### 4.2 프로그레스 애니메이션
- [x] Motion (Framer Motion) 설치 및 설정
- [x] 프로그레스 바 애니메이션 구현
- [x] 부드러운 카운트다운 효과

### 4.3 색상 변화 시스템
- [x] 시간 비율에 따른 색상 계산 로직
  - 100%~50%: 녹색 (#22c55e)
  - 50%~20%: 노란색 (#eab308)
  - 20%~0%: 빨간색 (#ef4444)
- [x] 색상 전환 애니메이션
...
### 5.1 타이머 상태 관리
- [x] Zustand store에 타이머 상태 추가
  - timeLeft, totalTime, isRunning, isPaused
- [x] setInterval 기반 카운트다운 로직
- [x] 타이머 정확도 최적화 (drift 보정)
...
- [x] 다음 사람 버튼
- [x] 리셋 버튼
- [x] 버튼 상태별 스타일링 (활성/비활성)

### 5.3 미팅 흐름 제어
- [x] 순서 셔플 버튼 (Fisher-Yates 알고리즘)
- [x] 미팅 시작 버튼 (현재 순서 유지)
- [x] 미팅 진행 상태 관리 (대기/진행중/완료)
...
### 6.1 현재/다음 발표자 UI
- [x] SpeakerDisplay 컴포넌트 생성
- [x] 현재 발표자: 크고 강조된 스타일
- [x] 다음 발표자: 작고 흐린 스타일 (opacity 50-70%)
- [x] 타이머 상단에 배치
...
### 7.1 오디오 설정
- [x] use-sound 라이브러리 설치
- [x] Web Audio API로 경고음 생성
- [x] 완료 알림음 생성

### 7.2 알림 로직
- [x] 10초 남았을 때 경고음 재생
- [x] 0초 (시간 종료) 시 완료 알림음 재생
- [x] 알림음 중복 재생 방지
...
### 7.4 설정 연동
- [x] 소리 On/Off 설정 반영
- [x] 브라우저 autoplay 정책 대응 (사용자 인터랙션 후 활성화)
...
### 8.1 설정 모달
- [x] 설정 모달 컴포넌트 생성
- [x] 기본 시간 조정 (30초 ~ 5분, 버튼 선택)
- [x] 연장 시간 단위 설정 (15초/30초/1분)
- [x] 소리 알림 On/Off 토글
- [x] 10초 경고음 On/Off 토글

### 8.2 설정 영속성
- [x] 설정값 Zustand store 저장
- [x] localStorage persist

### 8.3 UI/UX 마무리
- [x] 전체 레이아웃 정리
- [x] 반응형 디자인 확인
- [x] 접근성 검토 (키보드 네비게이션, aria 속성)
- [ ] 에러 상태 처리
...
| Sprint 4 | ✅ 완료 | 2025-12-14 |
| Sprint 5 | ✅ 완료 | 2025-12-14 |
| Sprint 6 | ✅ 완료 | 2025-12-14 |
| Sprint 7 | ✅ 완료 | 2025-12-14 |
| Sprint 8 | 🔄 진행중 (배포 대기) | - |
